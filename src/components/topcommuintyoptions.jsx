...
function QuestionTab({ activeTabs, setActiveTabs }) {
  ...
  const toggleTab = (tab) => {
    setActiveTabs((prevActiveTabs) => {
      const isActive = prevActiveTabs.includes(tab);
      const newActiveTabs = isActive 
        ? prevActiveTabs.filter((t) => t !== tab)
        : [...prevActiveTabs, tab];

      console.log(`${tab} is now ${isActive ? 'inactive' : 'active'}. Active tabs: ${newActiveTabs.join(', ')}`);
  ...
  return (
    <SearchAndTabsContainer>
      ...
      <ButtonAndTabsContainer>
        <TabsContainer>
          {['Newest', 'Unanswered', 'Bountied', 'Active'].map((tab) => (
            <Tab
              key={tab}
